...
<div class="card mb-4">
	...
	<div class="card-body">
		...
		<div class="row g-1">
			...
			<div class="xc-md-8 xc-xl-5 mb-2">{{ data.objekPajak.kecamatan ? data.objekPajak.kecamatan.nama : '-' }}</div>
			<div class="xc-md-3 xc-xl-2 mb-md-2 field-label">Kelurahan</div>
			<div class="xc-md-5 xc-xl-3 mb-2">{{ data.objekPajak.kelurahan ? data.objekPajak.kelurahan.nama : '-' }}</div>
		</div>
		<div class="row g-1">
		...
		<table class="table table-bordered">
			...
			<tbody>
				<tr v-for="(item, index) in data.pemilik">
					...
					<td>{{ item.nik }}</td>
					<td>{{ item.alamat }}</td>
					<td>{{ item.daerah ? item.daerah.nama : '-' }}</td> -->
					<td>{{ item.kelurahan ? item.kelurahan.nama : '-' }}</td>
					<td>{{ item.telp }}</td>
